...
الان میخواییم ما یه درخاست POST به localhost:300/api/contacts بزنیم و میخوایم یه مخاطب جدید اضافه کنیم با postman این عملیات رو انجام میدیم 

  <div align="center">
  <img  src="./img/validation-1.PNG"> 
</div>
ما گفتیم که حداقل باید name سه کاراکتر باشه و حداکثرهم 15 کاراکتر باشه ولیی ما دو کاراکتر "hh" نوشتیم 


  <div align="center">
  <img  src="./img/validation-2.PNG"> 
</div>

میبینیم که بهمون ارور داده و تو ترمینال هم گفته که name باید حداقل سه کارکتر باشه  پس اگه کمتر از سه و بیشتر از 15 کاراکتر باشن name , family بهمون ارور میده و نمیزاره contact اضافه بشه به دیتابیس 


هلا یه نکتته میخوایم فاصله هارو حذف کنیم مثلا ما گفتیم که name باید حداقل سه کاراکتر باشه خب ممکنه که کاربر بیاید بنویسه "a  " یه حرف رو بنویسه و دوتا فاصله بزاره خب فاصله ها هم کاراکتر حساب میشن و اینطوری میشه سه کاراکتر و تو دبتابیس هم ذخیره میشه خب چیزی که تو دیتابیس ذخیره میشه "a  " هست که ما چطوری جلوی اینکارو میگیریم ؟ ما باید اون فاصله های قبل یا بعد از اسم رو جذف کنیم و درنظر نگیریم با `trim : true` اینکارو انجام میده تو فایل مدل مون
...
export default Contact
```
یه نکته مهم اگه خاطرتون باشه گفتیم که هر تغیری که تو این فایل های داخل پوشه models بدیم باید حتما یه بار پروژه رو استوپ کنیم و دباره راهندازی کنیم مثلا اگه تو ترمینال vs code هستیم و پروژه درحال اجرا هست و ما تو این فایل تغیری ایجاد کردیم یه بار کلید Ctr , C رو فشار میدیم و سوال میکنه که میگه میخوایم متوقف کنی حرف y رو میزنیم و دباره دستور npm run dev رو میزنیم الان اگه کاربر بیاد برای name  بنویسه "   a" یا "a   " باز هم با ارور مواجه میشه چون الان دیگه ما با trim : true از اینکار جلو گیری کردیم اون فاصله های قبل یا بعدش رو درنظر نمیگیریم .


الان کاری که میخواییم انجام بدیم شماره تلفنی که کاربر میخواد تو فرم وارد میکنه رو میخوایم اعتبار سنجی کنیم مثلا اسم و فایلی و سن و جنسیتشو وارد کرد به درستی ولی شماره تلفن رو یه چیز نامعتبر وارد میکنه مثلا 0913 و ادامه شماره تلفن رو تکمیل نمیکنه بقیشو نمینویسه یا مینویسه 08 وقتی روی دکمه اضافه کردن میزنه ما نباید اجازه بدیم همچین مخاطبی اضافه بشه به دیتابیس باید تشخیص بدیم که شماره تلفنش نامعتبره وارد فولدر models میشیم و فایل contact.js میشیم که قبلا هم برای name , family یه اعتبار سنجی انجام داده بودیم 
...
export default Contact
```
برای phone هم اعتبار سنجی که نوشتیم  گفتیم که نوعش string باشه و بعد گفتیم که با این پترن ریجکس match باشه همخونی داشته باشه اون `/09\d{9}/` یه پترن ریجکسی هست که اگه نمیدونیم ریجکس چیه میتونیم دوره ریجکس رو نگاه کنیم برای درک بهتر دواقع یه الگو یه پترن رو مشخص کردیم برای شماره تلفن گفتیم که شماره تلفنی که کاربر وارد میکنه باید حتما با 09 شروع بشه و بعد از این باید 9 تا کاراکتر عددی هم بعدش وجود داشته باشه بعد 09 و trim : true هم دادیم که کاربر نتونه قبل یا بعد شماره تلفن فاصله تایپ کنه اگه وجود هم داشت اون فضاهای خالی رو حذف کنه الان ما اگه درخاست post بفرستیم تو postman به این ادرس localhost:3000/api/contact یا مثلا فرم رو پرکنیم ولی شماره رو با 09 شروع نکنیم یا کمتر از 9 کاراکتر بنویسیم شماره رو یا بیشتر از 9 کاراکتر باشه بهمون ارور میده.

---

> # قابلیت سرچ و فیلتر کردن مخاطب ها

یه قابلیتی که توی contact list یا همون لیست مخاطبین ما وجود داره اینه که بتونیم براساس نام و نام خانوادگی سرچ انجام بدیم و همچنین بر اساس جنسیت که مخاطب های ما اقا یا خانوم هستن فیلتر رو انجام بدیم مثلا اگه بزاریم روی female فقط مخاطب های خانوم رو نمایش میده بهمون یا برعکث یا اگه بزاریم روی all همه رو نشون میده یه قابلیت دیگه سرچ کردن نام یا نام خانوادگی هم براساس اینکه بتونیم جنسیت تعین کنیم یعنی فیلتر داشته باشیم هم بر اساس نام یا نام خانوادگی


  <div align="center">
   <img  src="./img/searchAndFiltringContacts.PNG"> 
  </div>
...
اگه ما وارد روت localhost:3000/api/contacts بشیم یه درخاست با متد get به این ادرس بزنیم لیست همه مخاطب هایی که داریم رو بهمون میده که تو دیتابیس وجود داره رو بصورت json بهمون میده که یه ارایه ای از ابجکتها هستن الان ما چطور یه کاری کنیم که مخاطب هایی رو دریافت کنیم که جنسیتشون مثلا male هست کافیه که تو سرچ اینطوری بنویسیم `localhost:3000/api/contacts?gen=male` ما از این علامت `?` به بعد رو میگیم query string این gen که تو این ادرس نوشتیم مخفف gender هست مثلا ما خودمون میتونیم هر اسمی بزاریم دلخواه خودمون تعریفش کنیم  الان اگه وارد این ادرس بشیم درخاست get بزنیم باید مخاطب هایی رو بهمون بده که جنسیتشون اقا هست . `پس ما به بعد از علامت سوال ؟ به بعد تو url از اون علامت `؟` به بعد رو میگیم query string` 

پس ما میتونیم بعد از ادرس روتی که همه contacts هامون رو داریم میتونیم علامت `?` بزاریم و جفت های کلید = مقدار بزاریم و حتی میتونیم از یکی بیشتز هم داشته باشیم یعنی چندین جفت کلید مقدار که با علامت `&` از هم جداشون میکنیم 
```js
...
//از api درخاست کردیم مخاطب هایی رو بهمون نشون بده که جنسیت اقا دارن و نام خانوادگیشون bahrami باشه هلا این gen , key که نوشتیم دلخواه هست میتونیم اسم کلید هایی دیگه ای رو هم بزاریم .
...
```
کاری که کردیم این بود که قبلا گفته بودیم که اگه کلید gen وجود داشت بیاد اینکارو کنه و اگه کلید search اسمی که کاربر سرچ میکنه وجود داشت چه تو name چه تو family بیاد اینکارو کنه هلا برای اولین شرط گفتیم که اگه هردو یعنی search , gen وجود داشته باشن بیاد اینکارو کنه و بقیه رو یعنی search , gen رو تو els if بعدی قرار داریم زمانی کد های داخل if(search && gen) اجرا میشن که ما یه همچنین چیزی داشته باشیم localhost:3000/api/contacts?gen=value&search=value اگه به همچنین روتی درخاست GET ارسال کنیم که شامل کلید های gen , search باشه این کدهای if اجرا میشن مثلا localhost:3000/api/contacts?gen=male&search=milad الان اگه ما تو دیتابیس مون دوتا داکیومنت دوتا مخاطب داشته باشیم مثلا هر دو با جنیسیت اقا باشن و هردو اسمشون milad باشه و فامیلیشون فرق داشته باشن باهم هردو رو میده .
---
